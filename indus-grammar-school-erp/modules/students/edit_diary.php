...
<div class="row mb-4 align-items-center d-print-none">
    <div class="col-sm-6">
        <h3 class="fw-bold text-secondary mb-0"><i class="fa-solid fa-pen-to-square me-2 text-primary"></i><?php echo ($diaryId > 0) ? 'Modify Diary Record' : 'Edit Diary Dashboard'; ?></h3>
    </div>
    <?php if ($diaryId > 0): ?>
        <div class="col-sm-6 text-sm-end mt-3 mt-sm-0">
            <a href="edit_diary.php" class="btn btn-sm btn-outline-secondary"><i class="fa-solid fa-arrow-left me-2"></i>Back to Dashboard</a>
        </div>
    <?php endif; ?>
</div>
...
<?php if ($diaryId > 0 && $diary): ?>
    ...
    <div class="card border-0 shadow-sm bg-white p-4" style="border-radius:12px;">
        <h5 class="fw-bold text-secondary mb-4 border-bottom pb-3"><i class="fa-solid fa-pen me-2 text-primary"></i>Edit Diary details</h5>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <select class="form-select" name="status" required>
                        <option value="Active" <?php echo ($diary['status'] === 'Active') ? 'selected' : ''; ?>>Active</option>
                        <option value="Draft" <?php echo ($diary['status'] === 'Draft') ? 'selected' : ''; ?>>Draft</option>
                    </select>
                </div>
                <div class="col-12">
                ...
                <div class="col-md-12">
                    ...
                    <?php if (!empty($diary['attachment'])): ?>
                        <div class="mb-2 p-2 border rounded bg-light d-flex align-items-center justify-content-between">
                            <span class="small text-secondary"><i class="fa-solid fa-file-arrow-down me-2 text-primary"></i>Current attachment: <strong><?php echo basename($diary['attachment']); ?></strong></span>
                            <a href="<?php echo APP_URL . '/' . $diary['attachment']; ?>" class="btn btn-sm btn-outline-primary" target="_blank">Download file</a>
                        </div>
                    <?php endif; ?>
                    <input type="file" class="form-control" name="attachment" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png">
                    <span class="small text-muted d-block mt-1">Leave blank to retain current attachment. Max: 5MB.</span>
                </div>
                
                <div class="col-12 text-end border-top pt-3 mt-4">
                    <button type="submit" id="updateBtn" class="btn btn-primary px-4"><i class="fa-solid fa-circle-check me-2"></i>Update</button>
                    <a href="edit_diary.php" class="btn btn-outline-secondary">Cancel</a>
                </div>
            </div>
        </form>
    </div>
    ...
    <div class="card border border-light shadow-sm bg-white p-4 mb-4 d-print-none" style="border-radius:12px;">
        ...
        <form method="GET" action="edit_diary.php" class="row g-3">
            ...
            <div class="col-md-10 text-end">
                ...
                <a href="edit_diary.php" class="btn btn-sm btn-outline-secondary">Reset</a>
            </div>
        </form>
    </div>
                    ...
                    <?php if (empty($diaries)): ?>
                        <tr>
                            <td colspan="11" class="text-center py-5 text-muted">
                                <i class="fa-solid fa-folder-open d-block fs-2 mb-2 text-secondary opacity-50"></i>
                                No diary entries found match search filters.
                            </td>
                        </tr>
                    <?php else: foreach ($diaries as $d): ?>
                        <tr>
                            <td><strong><?php echo date('M d, Y', strtotime($d['diary_date'])); ?></strong></td>
                            <td><?php echo sanitize($d['academic_type']); ?></td>
                            <td><?php echo displayValue($d['class']); ?></td>
                            <td><?php echo displayValue($d['section']); ?></td>
                            <td><strong><?php echo displayValue($d['subject']); ?></strong></td>
                            <td><span class="badge bg-secondary"><?php echo sanitize($d['diary_type']); ?></span></td>
